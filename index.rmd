...
casefig<-ggplot(data=AZPIMACOVID, aes(x=CaseDate, y=AvgIncrease), size=2, alpha=0.7)+
  ...
  scale_y_continuous(name = "Case Increase", breaks = seq(0,3000,500))+
...
The Case-to-Test ratio has been steadily rising for Arizona, indicating that the increase in cases is not simply due to increased testing. This ratio has risen in Pima County since re-opening on May 15, but has started to decline slightly in the past week. However, the ratio is stil well above the recommended ratio of <0.1.
...
Hospital COVID-19 specific metrics have been rapidly trending up in the last 3 weeks across Arizona. Hospital in-patient bed usage is currently at 85% and ICU bed usage is at 89% statewide (note that beds in use include both COVID and non-COVID patients).
...
hospfig<-ggplot(data = HOSPAZPIMA, aes(x = CaseDate, y = Patients/Beds), size=2, alpha=0.7) + 
  ...
  scale_y_continuous(name = "Patients or Beds", breaks = seq(0,2800,200))+
  scale_alpha(guide="none")+
  scale_size(guide="none")+
  guides(color=guide_legend(title=NULL))
