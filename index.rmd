...
Average daily case increases in Arizona and Pima County have rapidly increased since Memorial Day Weekend. Tuesday, June 16, had a record single-day case increase Pima County (385), while Friday, June 19, saw the highest day-to-day case increase in Arizona (3,246).
  
Note that typically it takes 10-14 days to see the effects of major policy changes or events. This is due to both the incubation period of COVID-19 (median of 4-5 days between exposure and symptom onset according to the CDC) and the reporting lag for new case datat (typically 4-7 days according to ADHS).  
...
Hospital COVID-19 specific metrics have been rapidly trending up in the last 3 weeks across Arizona. Hospital in-patient bed usage is currently at 84% and ICU bed usage is at 85% (note that beds in use include both COVID and non-COVID patients).
...
hospfig<-ggplot(data = HOSPAZPIMA, aes(x = CaseDate, y = Patients/Beds), size=2, alpha=0.7) + 
  ...
  theme(legend.position = 'bottom')+
  scale_x_date(name="Date", date_breaks = "1 week", date_minor_breaks = "1 day", date_labels = "%m/%d")+
  scale_y_continuous(name = "Patients or Beds", breaks = seq(0,2200,200))+
  scale_alpha(guide="none")+
  scale_size(guide="none")+
  guides(color=guide_legend(title=NULL))
